...
export class StudentprofileService {
  ...
  getEnrolledStudent(courseId: number): Observable<Employer[]> {
    return this.httpClient.get<Employer[]>(`${this.baseURL}/enrolled-user/${courseId}`);
  }  
  getEnrollmentCountbyTwoId(courseId: number, staffId: string): Observable<number> {
    return this.httpClient.get<number>(`${this.baseURL}/enrolled-time/${courseId}/${staffId}`);
  }
  ...
  getExamAnswercountCourses(staffId: string, courseId: string): Observable<ExamAnswerCountModel> {
    return this.httpClient.get<ExamAnswerCountModel>(`${this.baseURL}/examAnswerCount/${staffId}/${courseId}`);
  }
...
getCertificateCourses(staffId: string): Observable<StdentCourseModel[]> {
  return this.httpClient.get<StdentCourseModel[]>(`${this.baseURL}/certificateCourses/${staffId}`);
}

getEnrollCoursesview(staffId: string): Observable<CourseModelDTO[]> {
  return this.httpClient.get<CourseModelDTO[]>(`${this.baseURL}/enrollCourses/${staffId}`);
}
...
getEnrollmentCountNoDupli(courseId: string): Observable<number> {
  return this.httpClient.get<number>(`${this.baseURL}//course_countNg/${courseId}`);
}
