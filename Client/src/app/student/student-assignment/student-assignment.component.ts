...
export class StudentAssignmentComponent implements OnInit {
  ...
  //     const studentAssignment = assignments.find(a => a.studentId === this.studentStaffId);
  ...
  loadUploadedFile() {
    this.assignmentService.getStudentAssignmentsByAssignmentId(this.instructorAssignmentId).subscribe(assignments => {
      ...
      if (studentAssignment) {
        this.uploadedFile = {
          name: studentAssignment.fileName,
          type: studentAssignment.fileType,
          data: this.createDataURL(studentAssignment.data)
        };
      } else {
        this.uploadedFile = null;
      }
